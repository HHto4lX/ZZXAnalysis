...
void doNtuplesForMVA(TString inFile, TString outFile, float lumi, TString syear, TString massRegion, TString selection, TString finalstate)
{
  ...
  {  
    ...
    // save jet pT
    f_ptjet1 = JetPt_JESUp->at(d1_maxbtag);
    f_ptjet2 = JetPt_JESUp->at(d2_maxbtag);


    // build 2 jets tlorentzvectors
    TLorentzVector tlzvec_j1_;
    tlzvec_j1_.SetPtEtaPhiM(JetPt_JESUp->at(d1_maxbtag), JetEta->at(d1_maxbtag), JetPhi->at(d1_maxbtag), JetMass->at(d1_maxbtag));
    TLorentzVector tlzvec_j2_;
    tlzvec_j2_.SetPtEtaPhiM(JetPt_JESUp->at(d2_maxbtag), JetEta->at(d2_maxbtag), JetPhi->at(d2_maxbtag), JetMass->at(d2_maxbtag));
...
void prepareNtupleMVA_2bjet_JESUp()
{
  // --- mass region
  ...
  TString outputFilePath = "mvaNtuples_2bjet_JESUp_" + syear + "_" + massRegion + "_" + selection + "_" + finalstate;
