...
class PhotonFSRFiller : public edm::EDProducer {
 public:
  /// Constructor
  explicit PhotonFSRFiller(const edm::ParameterSet&);
...
PhotonFSRFiller::PhotonFSRFiller(const edm::ParameterSet& iConfig) :
  ...
  debug(iConfig.getUntrackedParameter<bool>("debug",false))
{
  ...
  if      (mode == "skip")        selectionMode = 0; // no FSR
    ...
    cout << "PhotonFSRFiller: mode " << mode << " not supported" << endl;
    abort();
  }
...
void
PhotonFSRFiller::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if (selectionMode!=0) {
    ...
    for (unsigned int i=0;i<pfCands->size();++i) {
      ...
      if (debug) cout << "PhotonFSRFiller: gamma:" << g->pt() << " " << g->eta() << " " << g->phi() << " SCVeto: " << SCVeto << endl;
...
DEFINE_FWK_MODULE(PhotonFSRFiller);
